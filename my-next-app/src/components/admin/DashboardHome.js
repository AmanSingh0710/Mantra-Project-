...
import {
  ...
  FaWallet, FaPercentage, FaFileInvoiceDollar, FaHourglassHalf
} from "react-icons/fa";
            ...
            <div className="grid grid-cols-2 gap-4">
              {stats?.popularProducts?.map((item, i) => (
                <div key={i} className="bg-gray-50 rounded-xl p-3 flex flex-col items-center border border-gray-100 hover:shadow-sm transition-shadow">
                  <img src={`http://localhost:5000/uploads/${item.thumbnail || item.image}`} alt={item.name} className="w-16 h-16 object-contain mb-2 rounded-md" />
