...
export default function TopPicks() {
  ...
  const fetchProducts = async () => {
    ...
    let url = `${BASE_URL}/Adminproducts/public`;
    if (userRole === "ADMIN") url = `${BASE_URL}/Adminproducts`;
  ...
  const handleReviewSubmit = async () => {
    const token = localStorage.getItem("token");
    if (!token) return toast.error("Please login to rate products");
    ...
    <section className="w-full bg-white py-10 md:py-20 overflow-x-hidden relative">
      ...
      {showReviewModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 text-black">
          <div className="bg-white w-full max-w-md rounded-lg p-6 relative animate-fadeIn shadow-2xl">
